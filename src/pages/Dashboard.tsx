...
import { mockMembers, mockDeaths, treasurySummary, mockContributions } from "@/data/mockData";
...
const Dashboard = () => {
  const activeMembers = mockMembers.filter((m) => m.status === "actif").length;
  const totalCovered = mockMembers.reduce((a, m) => a + m.totalCoveredPersons, 0);
  const ongoingDeaths = mockDeaths.filter((d) => d.status === "en_cours");
  const lateContributions = mockContributions.filter((c) => c.status === "non_payé" || c.status === "partiel");
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard icon={Landmark} label="Solde caisse" value={formatCFA(treasurySummary.totalBalance)} iconColor="bg-or-light text-accent" />
              ...
              ongoingDeaths.map((death) => (
                <div key={death.id} className="p-3 rounded-lg bg-secondary/50 border border-border/30">
                  <div className="flex items-center justify-between">
                    ...
                    <Badge variant="outline" className="bg-destructive-light text-destructive border-destructive/20 text-[10px]">
                      En cours
                    </Badge>
                  </div>
                  ...
                  <div className="mt-2 h-1.5 bg-border rounded-full overflow-hidden">
                    <div
                      ...
                      style={{ width: `${Math.min(100, (death.totalCollected / death.totalExpectedContributions) * 100)}%` }}
                    />
                  </div>
                </div>
              ))
          ...
          <CardContent>
            <div className="space-y-3">
              {[
                ...
                { icon: Coins, text: "Cotisation reçue — Esther YAO", sub: "1 000 FCFA", time: "23 fév 2026", color: "text-accent" },
                { icon: Coins, text: "Paiement partiel — Konan ASSI", sub: "1 000 / 2 000 FCFA", time: "25 fév 2026", color: "text-warning" },
              ].map((item, i) => (
      ...
      <Card className="border-border/50 bg-or-light/50">
        <CardContent className="pt-5 pb-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { label: "Total collecté", value: formatCFA(treasurySummary.totalContributionsCollected) },
              { label: "Versements effectués", value: formatCFA(treasurySummary.totalPayouts) },
              { label: "Réserves retenues", value: formatCFA(treasurySummary.retainedReserves) },
              { label: "Cotisations en attente", value: formatCFA(treasurySummary.pendingContributions) },
            ].map((item, i) => (
              <div key={i}>
